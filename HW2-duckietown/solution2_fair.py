...
def get_brick_borders(im):
    ...
    for i in range(1, n_comp):
        ...
        if size > 0.001 and (not np.any(components[:, 0] == i)) and (not np.any(components[0, :] == i)) and (not np.any(components[:, -1] == i)) and (not np.any(components[-1, :] == i)) and check_comp(components, i):
            if size > mx:
                mx = size
                mxi = i
...
class DontCrushDuckieTaskSolution(TaskSolution):
    ...
    def solve(self):
        ...
        condition = True
        while condition:
            img, reward, done, info = env.step([0.2, 0.5])
            duck_size = get_duck_size(img)
            # img in RGB
            # add here some image processing
        ...
        while not is_visible:
            img, reward, done, info = env.step([0.2, -0.2])
            is_visible, out = get_translation_matrix(img)

        condition = True
        while condition:
            img, reward, done, info = env.step([0.2, -1])
            is_visible, out = get_translation_matrix(img)
            condition = (is_visible)

        condition = True
        while condition:
            img, reward, done, info = env.step([0., 1.0])
            is_visible, out = get_translation_matrix(img)
            if is_visible:
                x, _ = out
                dq.append(x)
                x_cum = np.mean(dq)
            condition = (not is_visible)
